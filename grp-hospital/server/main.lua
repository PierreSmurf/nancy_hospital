...
RegisterServerEvent('grp-hospital:server:RequestBed')
AddEventHandler('grp-hospital:server:RequestBed', function()
    for k, v in pairs(beds) do
        if not v.taken then
            v.taken = true
            bedsTaken[source] = k
            TriggerClientEvent('grp-hospital:client:SendToBed', source, k, v)
            return
        end
    end
...
RegisterServerEvent('grp-hospital:server:RPRequestBed')
AddEventHandler('grp-hospital:server:RPRequestBed', function(plyCoords)
    ...
    for k, v in pairs(beds) do
        ...
        if distance < 3.0 then
            if not v.taken then
                v.taken = true
                foundbed = true
                TriggerClientEvent('grp-hospital:client:RPSendToBed', source, k, v)
                return
            else
...
RegisterServerEvent('grp-hospital:server:EnteredBed')
AddEventHandler('grp-hospital:server:EnteredBed', function()
    local src = source
    ...
    TriggerClientEvent('grp-hospital:client:FinishServices', src)
end)

RegisterServerEvent('grp-hospital:server:LeaveBed')
AddEventHandler('grp-hospital:server:LeaveBed', function(id)
    beds[id].taken = false
end)
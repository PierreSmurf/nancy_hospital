...
function CheckDamage(ped, bone, weapon)
    ...
    if Config.Bones[bone] ~= nil then
        if not BodyParts[Config.Bones[bone]].isDamaged then
            ...
            TriggerServerEvent('grp-hospital:server:SyncInjuries', {
                limbs = BodyParts,
...
RegisterNetEvent('grp-hospital:client:SyncBleed')
AddEventHandler('grp-hospital:client:SyncBleed', function(bleedStatus)
    isBleeding = tonumber(bleedStatus)
    DoBleedAlert()
end)

RegisterNetEvent('grp-hospital:client:FieldTreatLimbs')
AddEventHandler('grp-hospital:client:FieldTreatLimbs', function()
    for k, v in pairs(BodyParts) do
        v.isDamaged = false
        v.severity = 1
    end
...
RegisterNetEvent('grp-hospital:client:FieldTreatBleed')
AddEventHandler('grp-hospital:client:FieldTreatBleed', function()
    if isBleeding > 1 then
...
RegisterNetEvent('grp-hospital:client:ReduceBleed')
AddEventHandler('grp-hospital:client:ReduceBleed', function()
    if isBleeding > 0 then
        isBleeding = tonumber(isBleeding) - 1
...
RegisterNetEvent('grp-hospital:client:ResetLimbs')
AddEventHandler('grp-hospital:client:ResetLimbs', function()
    for k, v in pairs(BodyParts) do
        v.isDamaged = false
        v.severity = 0
    end
...
RegisterNetEvent('grp-hospital:client:RemoveBleed')
AddEventHandler('grp-hospital:client:RemoveBleed', function()
    isBleeding = 0
...
RegisterNetEvent('grp-hospital:client:UsePainKiller')
AddEventHandler('grp-hospital:client:UsePainKiller', function(tier)
    if tier < 4 then
...
RegisterNetEvent('grp-hospital:client:UseAdrenaline')
AddEventHandler('grp-hospital:client:UseAdrenaline', function(tier)
    if tier < 4 then
        onDrugs = 180 * tier
    end

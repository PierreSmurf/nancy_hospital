local hspLocs = {
    [1] = { ["x"] = 307.0818, ["y"] = -595.0646, ["z"] = 43.28398, ["h"] = 192.33229064941, ["name"] = "[E] Check In", ["fnc"] = "DrawText3DTest" },
}    
...
function LeaveBed()
    ...
    TriggerServerEvent('grp-hospital:server:LeaveBed', bedOccupying)
...
RegisterNetEvent('grp-hospital:client:RPCheckPos')
AddEventHandler('grp-hospital:client:RPCheckPos', function()
    TriggerServerEvent('grp-hospital:server:RPRequestBed', GetEntityCoords(PlayerPedId()))
end)

RegisterNetEvent('grp-hospital:client:RPSendToBed')
AddEventHandler('grp-hospital:client:RPSendToBed', function(id, data)
    bedOccupying = id
...
RegisterNetEvent('grp-hospital:client:SendToBed')
AddEventHandler('grp-hospital:client:SendToBed', function(id, data)
    bedOccupying = id
    bedOccupyingData = data
    ...
    Citizen.CreateThread(function ()
        ...
        TriggerServerEvent('grp-hospital:server:EnteredBed')
    end)
end)

RegisterNetEvent('grp-hospital:client:FinishServices')
AddEventHandler('grp-hospital:client:FinishServices', function()
                ...
                if hspDist < 3 then
                    ...
                    if IsControlJustReleased(0, 54) then
                        if (GetEntityHealth(PlayerPedId()) < 201) or (IsInjuredOrBleeding()) then
                            ...
                            exports["grp-taskbar"]:taskBar(1500, "Checking Credentials")
                                TriggerEvent('grp-ambulancejob:revive')
                                ClearPedTasks(PlayerPedId())
                                Citizen.Wait(1000)
                                TriggerServerEvent('grp-hospital:server:RequestBed')
                            else
